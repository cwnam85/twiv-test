...
{%- if pose.nsfw and affinity >= pose.unlock_affinity and pose.spot == characterStatus.spot.name %}
...
- {{ action.name }}: {{ action.description }} (Available ONLY in: {{ action.allowedPoses | join(", ") }}){% if action.notAllowedPoses and action.notAllowedPoses.length > 0 %} — Current Pose : {% if userRequestCharacterStatus == 'init' %}
  {{spots.defaultPose}}
  {% else %}
  {{characterLastPose}}
  {% endif %}, ⚠️ NOT available in: {{ action.notAllowedPoses | join(", ") }}. If user requests this action while in these poses, you MUST REFUSE and gently request to change other pose through dialog{% endif %}
  {%- endfor %}
